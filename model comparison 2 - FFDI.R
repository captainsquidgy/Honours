...
#FFDI 
gam_q <- qgam(log1p(FRP) ~ FFDI, qu = 0.8, data = df_af_sum2)
ef_q <- ggpredict(gam_q, terms=c("FFDI"))

#FFDI + SFL
gam_q <- qgam(log1p(FRP) ~ FFDI + SFL, qu = 0.8, data = df_af_sum2)
ef_q <- ggpredict(gam_q, terms=c("FFDI","SFL"))

#FFDI + VEG
gam_q <- qgam(log1p(FRP) ~ FFDI + VEG_GROUP, qu = 0.8, data = df_af_sum2)
ef_q <- ggpredict(gam_q, terms=c("FFDI", "VEG_GROUP"))

#FFDI + SFL + VEG
gam_q <- qgam(log1p(FRP) ~ FFDI + SFL + VEG_GROUP, qu = 0.8, data = df_af_sum2)
...
#s(FFDI) 
gam_q <- qgam(log1p(FRP) ~ s(FFDI, k=3), qu = 0.8, data = df_af_sum2)
ef_q <- ggpredict(gam_q, terms=c("FFDI"))

#s(FFDI) + SFL
gam_q <- qgam(log1p(FRP) ~ s(FFDI, k=3) + SFL, qu = 0.8, data = df_af_sum2)
ef_q <- ggpredict(gam_q, terms=c("FFDI","SFL"))

#s(FFDI) + VEG
gam_q <- qgam(log1p(FRP) ~ s(FFDI, k=3) + VEG_GROUP, qu = 0.8, data = df_af_sum2)
ef_q <- ggpredict(gam_q, terms=c("FFDI","VEG_GROUP"))

#s(FFDI) + SFL + VEG
gam_q <- qgam(log1p(FRP) ~ s(FFDI, k=3) + SFL + VEG_GROUP, qu = 0.8, data = df_af_sum2)
ef_q <- ggpredict(gam_q, terms=c("FFDI","SFL","VEG_GROUP"))
...
ggplot() +
  ...
  # Add raw data points
  geom_jitter(data = df_af_sum2, aes(x = FFDI, y = FRP, color = as.factor(SFL)), alpha=1, size=1) +
...
###################
#aic comparison
gam_1 <- qgam(log1p(FRP) ~ FFDI, qu = 0.8, data = df_af_sum2)

gam_2 <- qgam(log1p(FRP) ~ FFDI + SFL, qu = 0.8, data = df_af_sum2)

gam_3 <- qgam(log1p(FRP) ~ FFDI + VEG_GROUP, qu = 0.8, data = df_af_sum2)

gam_4 <- qgam(log1p(FRP) ~ FFDI + SFL + VEG_GROUP, qu = 0.8, data = df_af_sum2)

gam_5 <- qgam(log1p(FRP) ~ FFDI * SFL + VEG_GROUP, qu = 0.8, data = df_af_sum2)

gam_6 <- qgam(log1p(FRP) ~ s(FFDI, k=3), qu = 0.8, data = df_af_sum2)

gam_7 <- qgam(log1p(FRP) ~ s(FFDI, k=3) + SFL, qu = 0.8, data = df_af_sum2)

gam_8 <- qgam(log1p(FRP) ~ s(FFDI, k=3) + VEG_GROUP, qu = 0.8, data = df_af_sum2)

gam_9 <- qgam(log1p(FRP) ~ s(FFDI, k=3) + SFL + VEG_GROUP, qu = 0.8, data = df_af_sum2)

gam_10 <- qgam(log1p(FRP) ~ s(FFDI, k=3) * SFL + VEG_GROUP, qu = 0.8, data = df_af_sum2)
...
aic_tab <- as.data.frame(rbind(aic_1, aic_2, aic_3, aic_4, aic_5, aic_6, aic_7, aic_8, aic_9), row.names = c("log(FRP) ~ FFDI", "log(FRP) ~ FFDI + SFL", "log(FRP) ~ FFDI + Veg_Group" , "log(FRP) ~ FFDI + SFL + Veg_Group", "log(FRP) ~ FFDI * SFL + Veg_Group", "log(FRP) ~ s(FFDI)", "log(FRP) ~ s(FFDI) + SFL", "log(FRP) ~ s(FFDI) + Veg_Group" , "log(FRP) ~ s(FFDI) + SFL + Veg_Group"))
write.csv(aic_tab, file = "formal model selection/RESULTS/aictab_FFDI(QGAM).csv")
